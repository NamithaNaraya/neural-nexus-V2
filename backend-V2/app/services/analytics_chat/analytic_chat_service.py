...
# ===== NEW: Intelligent Entity Models =====
class EntityMatch(BaseModel):
    """Represents a potential entity match with confidence."""
    id: str
    name: str
    type: str
    confidence: float = Field(..., ge=0.0, le=1.0)  # 0-1 confidence score
    reason: str = ""  # Why this entity was matched

class DisambiguationContext(BaseModel):
    """Context for entity disambiguation."""
    query: str
    candidates: List[EntityMatch]
    folder_context: str = ""  # Type info from folder
    conversation_history: List[str] = []  # Previous questions for context
...
class AnalyticChatService:
    ...
    async def process_query(self, query: str, folder_id: Optional[str] = None, node_ids: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        # 2. Resolve entities with LLM-based disambiguation (NEW: context-aware)
        resolved_entities = []
        resolved_node_ids = []
        if entities_to_resolve:
            resolved_entities = await self._resolve_entities(
                entities_to_resolve, 
                folder_id,
                query_context=query  # Pass full query for context
            )
            resolved_node_ids = [r["id"] for r in resolved_entities]
            logger.info(f"[AnalyticChat] Resolved {len(resolved_entities)} entities with context awareness")
            for entity in resolved_entities:
                logger.info(f"  - {entity.get('reason', 'resolved')}")
    ...
    async def _resolve_entities(self, names: List[str], folder_id: Optional[str], query_context: str = "") -> List[Dict[str, Any]]:
        """
        NEW: Intelligent entity resolution using LLM-based disambiguation.
        Handles ambiguous names ("John" vs "Johnson") by understanding context.
        """
        if not names:
            return []
        
        logger.info(f"[Entity Resolution] Resolving {len(names)} entities with context awareness")
        
        async with self.driver.session() as session:
            # STEP 1: Find ALL candidate matches (exact + fuzzy)
            all_candidates = {}
            
            for name in names:
                query_find = """
                MATCH (n) 
                WHERE (n.name =~ ('(?i)^' + $exact_name + '$') OR n.name =~ ('(?i).*' + $name + '.*'))
                """
                if folder_id:
                    query_find += " AND n.folder_id = $folder_id"
                query_find += " RETURN n.id AS id, n.name AS name, coalesce(n.type, labels(n)[0]) AS type LIMIT 20"
            ...
            # STEP 2: Use LLM to disambiguate based on query context
            resolved = await self._llm_disambiguate_entities(names, all_candidates, query_context, folder_id)
            return resolved
    
    async def _llm_disambiguate_entities(self, names: List[str], candidates: Dict[str, List[Dict]], query_context: str, folder_id: Optional[str]) -> List[Dict[str, Any]]:
        """
        Uses LLM to intelligently pick the right entity when there are multiple matches.
        Example: "John" in context of "John Smith works with..." should match "John Smith", not "Johnson".
        """
        if not candidates:
            return []
        ...
        for name, matches in candidates.items():
            ...
            for i, m in enumerate(matches[:5], 1):
                candidate_text += f"  {i}. {m['name']} (Type: {m['type']})\n"
        
        # Ask LLM to pick the best match based on context
        llm_prompt = f"""
        Given the user's question context and list of candidate entities, pick the MOST LIKELY entity match.
        
        Question context: {query_context}
        Folder scope: {folder_id or 'global'}
        
        Candidates:{candidate_text}
        
        For each original name, respond with ONLY the best match number (1-5) and confidence (0.0-1.0).
        Example: {{
            "John": {{"candidate_index": 1, "confidence": 0.95}},
            "Smith": {{"candidate_index": 2, "confidence": 0.8}}
        }}
        ...
        try:
            decision = await self.ai.chat_json([{"role": "user", "content": llm_prompt}])
            ...
            for name, matches in candidates.items():
                if name not in decision:
                    ...
                    if matches:
                        resolved_entities.append({**matches[0], "confidence": 0.5, "reason": "fallback"})
                    continue
                
                choice = decision[name]
                idx = choice.get("candidate_index", 1) - 1
                confidence = choice.get("confidence", 0.5)
                
                if 0 <= idx < len(matches):
                    entity = matches[idx]
                    entity["confidence"] = confidence
                    entity["reason"] = f"LLM matched '{name}' to '{entity['name']}' with {confidence*100:.0f}% confidence"
                    resolved_entities.append(entity)
            
            logger.info(f"[Entity Resolution] LLM resolved to {len(resolved_entities)} entities")
            return resolved_entities
            
        except Exception as e:
            logger.warning(f"[Entity Resolution] LLM disambiguation failed: {e}. Using fallback.")
            # Fallback: return first match for each name
            fallback = []
            for name, matches in candidates.items():
                if matches:
                    m = matches[0]
                    m["confidence"] = 0.5
                    m["reason"] = "fallback - no LLM"
                    fallback.append(m)
            return fallback
